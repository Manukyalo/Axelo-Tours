...
export async function sendQuoteSubmissionAlert(quoteData: { quote_ref: string, company_name: string, destination: string, total_net: number }) {
  if (!process.env.RESEND_API_KEY) return;
  ...
  await resend.emails.send({
    from: FROM_EMAIL,
    to: ADMIN_EMAIL,
    subject: `New Group Quote Request: ${quoteData.quote_ref}`,
    html: `
      <div style="font-family: sans-serif; color: #333;">
        <h2>New Quote Submitted</h2>
        <p><strong>Partner:</strong> ${quoteData.company_name}</p>
        <p><strong>Reference:</strong> ${quoteData.quote_ref}</p>
        <p><strong>Destination:</strong> ${quoteData.destination}</p>
        <p><strong>Estimate:</strong> $${quoteData.total_net.toLocaleString()} USD</p>
        <p><a href="https://admin.axelotours.co.ke/quotes" style="color: #1A6B3A;">View in Admin Panel</a></p>
      </div>
    `
  });
}
