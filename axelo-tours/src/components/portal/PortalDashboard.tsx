...
import { AIPortalModal } from "@/components/portal/AIPortalModal";
...
export function PortalDashboard({ client, bookings }: PortalDashboardProps) {
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
               <button 
                ...
                className="flex items-center gap-2 bg-white border border-gray-200 text-gray-600 px-6 py-3 rounded-2xl font-bold text-sm hover:bg-gray-50 transition-colors group"
               >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {upcomingBookings.map((booking) => (
                  <BookingCard 
                    key={booking.id} 
                    booking={booking} 
                    onOpenZara={() => setIsAIModalOpen(true)}
                  />
                ))}
              </div>
            )}
  ...
  return (
    <div className="bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/40 border border-gray-100 overflow-hidden flex flex-col md:flex-row transition-transform hover:scale-[1.01] duration-300">
      <div className="w-full md:w-2/5 relative h-64 md:h-auto overflow-hidden">
        <Image 
          src={pkg.images?.[0] || "https://images.unsplash.com/photo-1516422317778-958bd73a7174?q=80&w=1000&auto=format&fit=crop"} 
          alt={pkg.name}
          fill
      ...
      <div className="flex-1 p-8 md:p-10 flex flex-col">
        ...
        <div className="mt-auto pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center gap-4">
           ...
           <button 
            onClick={onOpenZara}
            className="w-full sm:w-auto bg-gray-50 text-gray-600 px-8 py-4 rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-gray-100 transition-colors"
           >
